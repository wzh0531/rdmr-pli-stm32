"""Analyze the received S503/A2 and S504/A3 Proteus pair."""
...
REPORT = (
    ...
    / "phase5_p1_a2_a3_pair_audit.json"
)
FILES = {
    0: LOG_DIR / "PHASE5_P1_S501_A0_F1_P050_Z20_REV14.txt",
    1: LOG_DIR / "PHASE5_P1_S502_A1_F1_P050_Z20_REV14.txt",
    2: LOG_DIR / "PHASE5_P1_S503_A2_F1_P050_Z20_REV14.txt",
    3: LOG_DIR / "PHASE5_P1_S504_A3_F1_P050_Z20_REV14.txt",
}
N = 8000
...
def host_run(library: object, algorithm: int) -> dict[str, np.ndarray]:
    ...
    code = library.phase4_run(
        algorithm, 1, 0, 1, 20260727, 0.5, N,
...
def analyze_one(
    ...
    return {
        ...
        "log_sha256": sha256(FILES[algorithm]),
        "rows": len(rows),
        ...
        "endpoint_state_mismatches": int(np.count_nonzero(
            log_state != host["state"][indices]
        )),
...
def main() -> None:
    ...
    analyses = {}
    for algorithm, path in FILES.items():
        rows, stats = parse_log(path)
        analyses[algorithm] = analyze_one(
            algorithm, rows, stats, host_run(library, algorithm)
        )
    a2 = analyses[2]
    a3 = analyses[3]
    result = {
        ...
        "comparison": (
            "P1 A0-A3, with primary S504/A3 vs S503/A2 comparison; "
            "F1, PLI=0.50, 20 dB, seed=20260727"
        ),
        "per_algorithm": [
            analyses[algorithm] for algorithm in sorted(analyses)
        ],
        "a3_vs_a2": {
        ...
        "gates": {
            "log_integrity": "PASS",
            "numeric_faults": "PASS",
            "host_proteus_output_snr_le_0.10_db": "PASS",
            "host_proteus_endpoint_frequency_mae_le_0.05_hz": "PASS",
            "tracker_calls_difference_le_1": "PASS",
            "state_sequence": (
                "PASS for A2; WARN for A3: 2/160 endpoint mismatches "
                "at n=4700 and n=4850"
            ),
            "a3_mean_cycles_below_a2": "PASS",
            "maximum_sample_below_72000_cycles": "FAIL",
            "zero_deadline_violations": "FAIL",
        },
        "evidence_boundary": {
