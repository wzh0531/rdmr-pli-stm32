...
foreach ($scenario in $scenarios) {
    ...
    $stem = "PHASE5_PHYSICAL_{0}_S{1}_{2}_{3}_P{4}_Z20_REV15" -f `
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Physical build failed for scenario $($scenario.Id)"
    }
}

Write-Output "Built $($scenarios.Count) Phase-5 physical F103 core scenarios."
